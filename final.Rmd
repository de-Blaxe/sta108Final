...
validationData <- originalData[!(1:nrow(originalData) %in% sampledRows), ]
...
subsets <- regsubsets(glyhb~., data=originalCategorizedData, nbest = 10, method = "exhaustive", nvmax = 20)
summary(subsets)

library(knitr)
data <- originalCategorizedData
Y <- originalCategorizedData$glyhb
models <- regsubsets(glyhb~., data=originalCategorizedData, nbest = 10, method = "exhaustive", nvmax = 20)
